...
import type { GameRoom } from "./server/server";
import { createServerApi } from "./server/api/debug";
import { createGameWs } from "./server/ws/game-ws";
...
export function createApp(room: GameRoom) {
  return new Elysia()
    .use(createServerApi(room))
    .use(createGameWs(room))
    .get("/", async () => {
      const indexFile = Bun.file(CLIENT_INDEX_URL);

      if (!(await indexFile.exists())) {
        return new Response(
          "客户端资源缺失，请在项目根目录运行 `pnpm run build`",
          {
            status: 503,
          },
        );
